...
class InkboxAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        # Platform("inkbox") via _missing_() — no core enum entry needed.
...
def check_requirements() -> bool:
    """Return True when SDK + aiohttp are importable and required env vars are set."""
    if not check_inkbox_requirements():
        return False
    return bool(os.getenv("INKBOX_API_KEY") and os.getenv("INKBOX_IDENTITY"))


def validate_config(config) -> bool:
    """Return True when the config has enough info to attempt a connect."""
    extra = getattr(config, "extra", {}) or {}
    api_key = os.getenv("INKBOX_API_KEY") or extra.get("api_key", "")
    identity = os.getenv("INKBOX_IDENTITY") or extra.get("identity", "")
    return bool(api_key and identity)


def is_connected(config) -> bool:
    """Configuration-only liveness check used by ``hermes gateway status``."""
    return validate_config(config)


def _env_enablement() -> Optional[Dict[str, Any]]:
    """Seed ``PlatformConfig.extra`` from ``INKBOX_*`` env vars.

    Returns ``None`` when Inkbox isn't minimally configured so the
    registry skips auto-enabling. ``home_channel`` becomes a proper
    ``HomeChannel`` dataclass on ``PlatformConfig``; every other key is
    merged into ``extra``.
    """
    api_key = os.getenv("INKBOX_API_KEY", "").strip()
    identity = os.getenv("INKBOX_IDENTITY", "").strip()
    if not (api_key and identity):
        return None
    seed: Dict[str, Any] = {"api_key": api_key, "identity": identity}

    # Optional knobs — only seed when explicitly set so adapter defaults win
    for env_name, extra_key in (
        ("INKBOX_SIGNING_KEY", "signing_key"),
        ("INKBOX_BASE_URL", "base_url"),
        ("INKBOX_HOST", "host"),
        ("INKBOX_PUBLIC_URL", "public_url"),
        ("INKBOX_TUNNEL_NAME", "tunnel_name"),
    ):
        value = os.getenv(env_name, "").strip()
        if value:
            seed[extra_key] = value

    port = os.getenv("INKBOX_LISTEN_PORT", "").strip()
    if port:
        try:
            seed["port"] = int(port)
        except ValueError:
    ...
    require_sig = os.getenv("INKBOX_REQUIRE_SIGNATURE", "").strip().lower()
    if require_sig:
        seed["require_signature"] = require_sig in ("1", "true", "yes")

    home = os.getenv("INKBOX_HOME_CHANNEL", "").strip()
    if home:
        seed["home_channel"] = {
            "chat_id": home,
...
async def _standalone_send(
    ...
    *,
    ...
    media_files=None,  # noqa: ARG001
    force_document: bool = False,  # noqa: ARG001
) -> Dict[str, Any]:
    """Out-of-process Inkbox send for ``deliver=inkbox`` cron jobs."""
    extra = dict(getattr(pconfig, "extra", {}) or {})
    return await send_inkbox_direct(
        extra=extra,
        chat_id=chat_id,
        message=message,
        thread_id=thread_id,
...
def interactive_setup() -> None:
    """``hermes gateway setup`` wizard — credentials, allowlist, home channel."""
    ...
    if existing_key:
        print_info("Inkbox: already configured (API key set)")
        if not prompt_yes_no("Reconfigure Inkbox?", False):
            return

    print_info("Inkbox gives your agent a real email + phone + persistent contact list.")
    print_info("   Sign up at https://inkbox.ai/console and create an API key + identity.")
    print()

    api_key = prompt("Inkbox API key", password=True, default="")
    if not api_key:
        print_warning("API key is required — skipping Inkbox setup")
        return
    save_env_value("INKBOX_API_KEY", api_key.strip())

    identity = prompt(
        "Inkbox identity handle (e.g. your agent's name)",
        default=get_env_value("INKBOX_IDENTITY") or "",
    )
    if not identity:
        print_warning("Identity handle is required — skipping Inkbox setup")
        return
    save_env_value("INKBOX_IDENTITY", identity.strip())

    print()
    print_info("🔑 HMAC signing key (strongly recommended)")
    print_info("   Verifies inbound webhooks were actually sent by Inkbox.")
    print_info("   Without this, anyone who knows your public URL could spoof messages.")
    signing_key = prompt("Signing key (leave blank to skip — not recommended)", password=True, default="")
    if signing_key:
        save_env_value("INKBOX_SIGNING_KEY", signing_key.strip())
        save_env_value("INKBOX_REQUIRE_SIGNATURE", "true")
    else:
        save_env_value("INKBOX_REQUIRE_SIGNATURE", "false")
        print_warning("⚠️  Webhook signature verification disabled — anyone who reaches your public URL can deliver messages.")

    print()
    print_info("🔒 Access control: restrict who can talk to the bot")
    allow_all = prompt_yes_no("Allow anyone to message the agent? (dev only)", False)
    if allow_all:
        save_env_value("INKBOX_ALLOW_ALL_USERS", "true")
        save_env_value("INKBOX_ALLOWED_USERS", "")
        print_warning("⚠️  Open access — any contact who reaches your address can command the bot.")
    else:
        save_env_value("INKBOX_ALLOW_ALL_USERS", "false")
        allowed = prompt(
            "Allowed contacts (comma-separated emails / phone numbers / contact ids)",
            default=get_env_value("INKBOX_ALLOWED_USERS") or "",
        )
        if allowed:
            save_env_value("INKBOX_ALLOWED_USERS", allowed.replace(" ", ""))
            print_success("Allowlist configured")
        else:
            save_env_value("INKBOX_ALLOWED_USERS", "")
            print_info("No contacts allowed — the bot will ignore all messages until you add some.")

    print()
    print_info("📬 Home channel (optional)")
    print_info("   Default contact for cron / broadcast delivery.")
    home = prompt(
        "Home channel contact id (or empty)",
        default=get_env_value("INKBOX_HOME_CHANNEL") or "",
    )
    if home:
        save_env_value("INKBOX_HOME_CHANNEL", home.strip())

    print()
    print_success("Inkbox configuration saved to ~/.hermes/.env")
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'hermes-agent[inkbox]'",
        ...
        # SMS chunked in send() against SMS_MAX_LENGTH; email/voice unbounded.
        max_message_length=0,
        emoji="☤",
        # chat_ids may carry raw emails / phone numbers.
        pii_safe=True,
        allow_update_command=True,
        platform_hint=(
            "You are reachable via Inkbox — email, SMS, and live phone calls "
            "all route to one session per contact. Voice replies are spoken "
            "via TTS, so keep them short and natural. SMS is plain text only "
            "(no markdown, ~1600 char hard cap, segments billed per 160 "
            "chars). Email accepts plain text; pass `subject` in metadata "
