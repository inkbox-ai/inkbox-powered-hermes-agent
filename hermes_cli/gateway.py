...
_PLATFORMS = [
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. (Local dev) Get a free ngrok authtoken at dashboard.ngrok.com so the",
            "   adapter can expose its webhook + call-WS endpoints publicly.",
            "5. Hermes patches the identity's mailbox + phone number to point at the",
            "   adapter's tunnel on every gateway start — you do not need to set",
            "   webhook URLs manually in the Inkbox console.",
        ],
        "vars": [
            ...
            {"name": "NGROK_AUTHTOKEN", "prompt": "Ngrok authtoken (skip if INKBOX_PUBLIC_URL is set)", "password": True,
             "help": "Optional — only needed if INKBOX_PUBLIC_URL is not provided. Free tokens at dashboard.ngrok.com."},
            {"name": "INKBOX_PUBLIC_URL", "prompt": "Public HTTPS URL (skip to use ngrok)", "password": False,
             "help": "Optional override — set when running behind a reverse proxy or hosted tunnel."},
